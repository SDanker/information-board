...
def login(payload: LoginRequest, request: Request, db: Annotated[Session, Depends(get_db)]) -> TokenResponse:
    ...
    if too_many_attempts(f"login:{client_ip}", limit=settings.login_max_attempts, window_seconds=settings.login_window_seconds):
        raise HTTPException(
    ...
    if user is None or not user.is_active or not password_ok:
        record_audit(db, user, "login_failed", "auth", entity_id=payload.username, ip=client_ip)
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail=_("Incorrect username or password"))
    record_audit(db, user, "login", "auth", entity_id=str(user.id), ip=client_ip)
