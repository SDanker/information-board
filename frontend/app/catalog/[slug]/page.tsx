...
/** The page a screen's QR code opens: everything shown on that screen, plus the archive of finished publications. */
export default function CatalogPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <main className="library-page">
      ...
      <LibraryTabs value={tab} onChange={setTab} currentLabel="library.tab.screen" />
      {tab === "archived" ? (
        <ArchivedLibrary />
      ) : (
        <>
