...
const CREATABLE_KINDS: ContentKind[] = ["ANNOUNCEMENT", "IMAGE", "VIDEO", "DOCUMENT", "EXCEL", "PPTX", "EMERGENCY"];
  ...
  return (
    <AdminShell page="content" eyebrow={t("eyebrow.content")}>
      ...
      <section className="screen-cards">
        {items.length === 0 && <div className="empty-state"><FileText /><p>{t("content.empty")}</p></div>}
        {items.map((item) => {
          ...
          return (
            <article className={`screen-card ${isActiveEmergency ? "emergency-live" : ""}`} key={item.id}>
              ...
              <div className="screen-card-actions">
                ...
                {isEmergency && <button onClick={() => setMediaFor(item)}><ImageIcon size={16} /> {t("content.media")}</button>}
                ...
                {item.library_visibility !== "PRIVATE" && <button onClick={() => openShare(item)}><Share2 size={16} /> {t("content.share")}</button>}
        ...
        <div className="modal-backdrop" role="presentation">
          <form className="modal-card" onSubmit={submit}>
            ...
            <div className="form-grid">
              ...
              {kind === "EMERGENCY" && (
                <>
                  <label className="full">{t("content.form.address")}<input name="address" placeholder={t("content.form.addressPlaceholder")} /></label>
            ...
            {kind === "EMERGENCY" && (
              <div className="emergency-sections">
                ...
                <small className="form-hint">{t("content.form.mediaHint")}</small>
              </div>
            )}
